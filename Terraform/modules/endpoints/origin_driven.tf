...
# # Explanation: Public feed is cacheable—but only if the origin explicitly says so. arcanum demands consent.
...
# # Explanation: Everything else under /api is dangerous by default—arcanum disables caching until proven safe.
# ordered_cache_behavior {
#   path_pattern           = "/api/*"
#   target_origin_id       = "${var.project_name}-alb-origin01"
#   viewer_protocol_policy = "redirect-to-https"
