...
REPORT_READY_SNS_TOPIC_ARN = os.environ["REPORT_READY_SNS_TOPIC_ARN"]
...
def bedrock_generate(report_prompt: str):
    ...
    # Fallback so you can still inspect unexpected response bodies
...
| filter @message like /ERROR|Error|Exception|Traceback|DB|db|timeout|Timeout|refused|Access denied/
| sort @timestamp desc
| limit 50
""".strip(),
        start_ts, end_ts
...
fields @timestamp, @message
| filter @message like /ERROR|Error|Exception|Traceback|DB|db|timeout|Timeout|refused|Access denied/
| stats count() as errors by bin(1m) as time_bin
| sort time_bin asc
""".strip(),
        start_ts, end_ts
    )
    ...
    incident_id = f"arcanum-{datetime.datetime.utcnow().strftime('%Y%m%d-%H%M%S')}"
    evidence = {
...
"""
    ...
    sns.publish(TopicArn=REPORT_READY_SNS_TOPIC_ARN, Subject=f"IR Report Ready: {incident_id}", Message=msg)

    return {"ok": True, "incident_id": incident_id, "report_s3": f"s3://{REPORT_BUCKET}/{md_key}"}


def INCIDENT_TEMPLATE():
    return f"""# Incident Report: {{{{incident_id}}}} — {{{{title}}}}
...
- Secrets Manager: {SECRET_ID} (host/port/dbname/username/password)
- Notes on drift:
